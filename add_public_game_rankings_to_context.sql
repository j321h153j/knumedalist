      ...
      select coalesce(
        jsonb_agg(
          jsonb_build_object(
            ...
            'record_value', gr.record_value
          )
          order by g.scheduled_start_time, g.title, gr.rank_order, gr.id
        ),
        '[]'::jsonb
      )
...
select
  public.get_public_event_context() ? 'game_rankings' as has_game_rankings,
  jsonb_array_length(coalesce(public.get_public_event_context()->'game_rankings', '[]'::jsonb)) as game_ranking_count;
